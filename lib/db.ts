...
export class StorageService {
  async initializeDb(): Promise<void> {
    ...
    if (moduleCount === 0) {
      ...
      console.log('[v0] Seeded', SEED_MODULES.length, 'modules');
    }

    if (habitCount === 0) {
      await db.habits.bulkAdd(SEED_HABITS);
      console.log('[v0] Seeded', SEED_HABITS.length, 'habits');
    }

    if (blockCount === 0) {
      await db.scheduleBlocks.bulkAdd(SEED_SCHEDULE_BLOCKS);
      console.log('[v0] Seeded', SEED_SCHEDULE_BLOCKS.length, 'blocks');
    }
    ...
    const fresh = await db.scheduleBlocks.count();
    console.log('[v0] Database initialized, blocks:', fresh);
  }
