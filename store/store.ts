...
interface LifeOsStore {
  ...
  // Habit Management
  addHabit: (habit: Pick<Habit, 'name' | 'type' | 'targetFrequency'>) => Promise<void>;
...
export const useLifeOsStore = create<LifeOsStore>((set, get) => ({
  ...
  logSession: async (sessionData: Omit<Session, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    set({ sessions });
    // Only show toast for meaningful user-triggered actions, not block_complete (has its own UI)
    if (session.type !== 'block_complete') {
      toast.success(`Session logged (${session.durationMinutes}m)`);
    }
    return session.id;
  },
  ...
  // ===== Habits =====
  addHabit: async ({ name, type, targetFrequency }) => {
    const now = new Date();
    const habit: Habit = {
      id: uuidv4(),
      name,
  ...
  deleteHabit: async (habitId: string) => {
    ...
    const habits = await storage.getHabits();
    set({ habits });
    toast.success(habit ? `"${habit.name}" deleted` : 'Habit deleted');
  },
  ...
  checkInHabit: async (habitId: string) => {
    ...
    const now = new Date();
    const lastCheckin = habit.lastCompletedDate ? new Date(habit.lastCompletedDate) : null;

    // Fixed streak logic: consecutive = completed yesterday (calendar day)
    ...
    const newStreak = isConsecutiveDay ? habit.currentStreak + 1 : 1;
    const updatedHabit = {
      ...habit,
    ...
    if (newStreak > 1) {
      toast.success(`${habit.name} — 🔥 ${newStreak} day streak!`);
    } else {
      toast.success(`${habit.name} checked in ✓`);
    }
  },
  ...
  updateWeeklyReview: async (review: WeeklyReview) => {
    ...
    // Refresh the list so Past Reviews updates immediately
    const weeklyReviews = await storage.getWeeklyReviews();
    set({ weeklyReviews });
  ...
  calculateExecutionScore: (startDate: Date, endDate: Date) => {
    ...
    while (cursor.getTime() <= endTime) {
      days.push(cursor.getDay()); // we need dayOfWeek per date
      cursor.setDate(cursor.getDate() + 1);
    }

    // Count total scheduled fixed blocks across the date range
    // For each date in range, count blocks scheduled for that day of week
    let totalBlocks = 0;
    const cursorDate = new Date(startDate);
    while (cursorDate.getTime() <= endTime) {
      // dayOfWeek: 0=Monday schema
      const jsDay = cursorDate.getDay(); // 0=Sun
      const dayOfWeek = (jsDay + 6) % 7; // convert to 0=Monday
      const blocksForDay = store.baseScheduleBlocks.filter(
        (b) => b.dayOfWeek === dayOfWeek && b.timeKind === 'fixed'
      );
      totalBlocks += blocksForDay.length;
      cursorDate.setDate(cursorDate.getDate() + 1);
    }

    if (totalBlocks === 0) return 0;

    // Count completed blocks (block_complete sessions) in range
    const completedCount = store.sessions.filter(
      (s) =>
        s.type === 'block_complete' &&
        s.date.getTime() >= startTime &&
        s.date.getTime() <= endTime
    ).length;

    return Math.round(Math.min((completedCount / totalBlocks) * 100, 100));
  },
